...
`modal-devin` supplies the orchestrator for Modal. It connects an Outposts pool to
isolated [Modal Sandboxes](https://modal.com/docs/guide/sandboxes) and manages each
session from startup through cleanup.

```mermaid
flowchart LR
    D["Devin Cloud<br/>planning and session UI"] --> P["Outposts pool"]
...
- Watches the configured Outposts pool for work.
...
so use least-privilege credentials and separate pools when trust requirements differ.

## Network requirements

Workers need outbound HTTPS access to Devin. They do not require inbound ports, a
public IP, or a connection initiated from Devin Cloud.

## Operational model

One generated application represents one pool deployment. Give deployments unique,
