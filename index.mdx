...
1. The **worker** executes one Devin session. `modal-devin` runs it inside a Modal
   Sandbox.
2. The **orchestrator** finds queued sessions and manages their compute lifecycle.
...
```mermaid
flowchart LR
    D["Devin Cloud"] --> P["Your Outposts pool"]
...
<Columns cols={2}>
  <Card title="Deploy an existing pool" icon="play" href="/tutorials/first-worker">
    Go from a pool ID to a verified Modal deployment.
  </Card>
  <Card title="Create a new pool" icon="wand-magic-sparkles" href="/tutorials/create-a-pool">
    Create the pool, credential, and application interactively.
  </Card>
...
- Project source, tools, and dependencies.
- Modal Secrets available to your functions and workloads.
- CPU, memory, region, retries, and schedule.
- Session duration and snapshot retention.
...
and cleanup. Those lifecycle mechanics do not need to be reimplemented in your
application.
...
- An existing Outposts pool ID, or the Devin CLI to create one.
