...
description: Configure one Outposts pool, build its image, and run its Modal entrypoints.
icon: gears
---

`Worker` is the supported runtime interface for one Devin Outposts pool.
...
Finds queued sessions for the configured pool and starts eligible Modal invocations.
