...
`doctor` has no local Devin CLI prerequisite: `init` creates outposts with a direct
Devin Outposts API call, and only the deployed worker image needs the Devin CLI at runtime.
