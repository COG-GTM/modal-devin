...
| --- | --- | ---: | --- |
| `scheduler_interval_seconds` | `int` | `30` | How often the generated schedule checks for work |
| `session_timeout_seconds` | `int` | `1800` | Maximum Devin worker duration |
...
| `sidecar_ready_timeout_seconds` | `int` | `60` | Maximum credential-boundary startup wait |
...
Durations, counts, and timeouts must be positive, except that retry delay may be zero
and snapshot retention may be `None`. Log level is case-insensitive and accepts
