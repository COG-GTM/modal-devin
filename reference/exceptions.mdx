...
The installed Modal SDK does not provide a capability required by this release. Run
...
behavior. Check the credential, pool, configured API URL, and preceding network error.

## `OutpostsProtocolError`

Devin returned a response that this release could not handle safely. Check release
compatibility and the original response error; do not treat the affected invocation
as successful.

## `SessionStatusUnknownError`

The worker exited, but `modal-devin` could not confirm a safe final session state. It
